--[[
	...
	- Функуий скрипта:
	- ставит стоп-ордер если его нет (размер можно настраивать на каждый инструмент отдельно в STOP_TABLE)
	- изменят колличество стоп-ордеров если выходишь частями (лимитками или маркетом)
	- удаляет стоп-ордера по интсрументу (другие не трогает) если вы вышли из позиции целиком (лимитками или маркетом)
	- удаляет все лимитные-ордра по интсрументу (другие не трогает) если позиция закрылась по стоп-ордеру
...
STOP_INDENT 			= 200; 									-- Стоп-ордера (по умолчанию) измеряется ценой не пунктами
STOP_TABLE 				= {											-- Массив БАЗОВЫХ АТИВАХ Стопов (по необходимости добавлять или удалить)
	BR  = 0.2,															-- Стоп-ордер BR
	RTS = 200,															-- Стоп-ордер RTS
	Si  = 50,																-- Стоп-ордер Si
};
...
-- Функция main() основной логики скрипта, работает в отдельном потоке
function main()
	local array_class_code = {}; -- Рабочий массив, с которым мы работаем

	-- Заплоняем массив с посоянными данными, которые не нужно перезаисыать
	for sec_code in string.gmatch(getClassSecurities(CLASS_CODE), "(%w+)") do
		...
		-- Изменяем значение стоп-ордера по умолчанию, на значение стоп-ордера из массива STOP_TABLE
		-- Узнаем какой базовый актив у инструмента
		local base_active =  getSecurityInfo(CLASS_CODE, sec_code).base_active_seccode;
		-- проверка на существоваиня стопа в STOP_TABLE, если есть то устанавливаем знаяение из STOP_TABLE
		if (STOP_TABLE[base_active]) then
			array_class_code[sec_code].stop_indent = STOP_TABLE[base_active];
		end;
	end;
	-- Вечный цикл работы логики скрипта (Is_Run = true), пока не нажата кнопка ОСТАНОВИТЬ в терминале (сработает function OnStop() и поменяет значение Is_Run = false)
	while Is_Run do
		sleep(1000); -- одна сикунда = 1000
		-- Проверяем если есть соединение с сервером
		if isConnected() then
      main_BODY(array_class_code); -- передаем рабочий массив в тело скрипта
        else
...
--[[
	Функция main_BODY() - логика функций main() выведена в однелную функцию для удобства восириятия кода
	Логика посторена на перезаписываемые значеня в рабочем массиве,
	которые проверяються условиями при каждом проходе цикла,
	если все устраивает ничего не делаем, если нет то удаляем или добавляем стоп-ордера.
--]]
function main_BODY(array_class_code)
	-- Добовляем в массив перезаписываемые значение (эти значения перезаписываемые при каждом проходе цикла)
	...
	for key, val in pairs(array_class_code) do
		...
		if (val.pos_sum ~= 0  and val.stop_sum ~= 0 and math.abs(val.pos_sum) ~= math.abs(val.stop_sum)) then
			...
			-- Ставим новую стоп заявку со старым значеним
